const socketController = (socket) => {
    ...
    console.log(`Cliente ${socket.id} conectado en ${process.env.WEBSOCKETPORT}`);  //Estos 'id' son muy volátiles y no es muy correcto usarlos para nada especial. Luego se asociarán los clientes a salas y eso es lo más correcto, gestionarlo en las salas.
    ...
    socket.on('enviar-mensaje', (payload, callback) => {
        console.log(`Mensaje recibido en ${process.env.WEBSOCKETPORT}: ${JSON.stringify(payload, null, 2)} de ${socket.id}`);
